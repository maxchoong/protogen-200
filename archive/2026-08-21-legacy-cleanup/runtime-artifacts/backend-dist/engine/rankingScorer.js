...
export class RankingScorer {
     ...
     */
    static genreMatchScore(itemGenres, preferredGenres) {
        if (preferredGenres.length === 0 || itemGenres.length === 0)
            return 0;
        let score = 0;
        // Count primary matches (exact matches)
        const primaryMatches = itemGenres.filter(g => preferredGenres.some(pg => pg.toLowerCase() === g.toLowerCase())).length;
     ...
     */
    static moodMatchScore(plotText, preferredMoods, moodStrength, boostedMoods = [], reducedMoods = []) {
        if (!plotText || plotText.length === 0 || preferredMoods.length === 0)
            return 0;
     ...
     */
    static talentMatchScore(talentScore) {
     ...
     */
    static popularityScore(voteCount) {
        if (voteCount === undefined || voteCount === null || voteCount === 0)
            return 0.5; // neutral default
     ...
     */
    static recencyBoost(year, currentYear = 2026) {
     ...
     */
    static calculateCompositeScore(title, preferences, config = {}) {
        const finalConfig = {
        ...
        const moodScore = Number(this.moodMatchScore(title.plot, preferences.mood || [], preferences.moodStrength, preferences.boostedMoods || [], preferences.reducedMoods || [])) || 0;
        ...
        const popularitySignal = preferences.noveltyIntent
            ? (1 - popularityScore)
            : popularityScore;
        const composite = genreScore * finalConfig.weights.genre +
     ...
     */
    static rankTitles(titles, preferences, config = {}) {
     ...
     */
    static explainScore(title, factors) {
        return [
            `Title: ${title.title}`,
