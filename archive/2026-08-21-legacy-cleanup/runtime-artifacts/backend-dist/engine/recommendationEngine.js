...
import { RankingScorer } from './rankingScorer.js';
/**
...
export class RecommendationEngine {
    constructor() {
        ...
        this.referenceTitlesCache = new Map();
    ...
    async searchWithTmdbPrimary(searchTerms, typeFilter, discoveryMode, preferredGenres = []) {
        if (!tmdbClient.isEnabled()) {
            return [];
        }
        const includeMovies = typeFilter !== 'series';
        const includeTV = typeFilter !== 'movie';
        const shouldUseGenreDiscover = preferredGenres.length > 0 &&
            (discoveryMode === 'mood' || discoveryMode === 'reference' || discoveryMode === 'mixed');
        const tmdbResultSets = [];
        if (shouldUseGenreDiscover) {
            const discoverResults = await tmdbClient.discoverByGenres(preferredGenres.slice(0, 3), {
                includeMovies,
                includeTV,
                excludeAdult: true
            });
        ...
        if (searchTerms.length > 0) {
            const tmdbSearches = await Promise.all(searchTerms.map(term => tmdbClient.searchTitles(term, {
                includeMovies,
                includeTV,
                excludeAdult: true
            })));
        ...
        const deduped = Array.from(new Map(tmdbResultSets
            .flat()
            .map(item => [`${item.media_type}:${item.id}`, item])).values());
        const limited = deduped.slice(0, 30);
        const converted = await Promise.all(limited.map(async (item) => this.convertTmdbResult(item)));
        return converted.filter(item => !!item.title && !!item.plot);
    }
    async searchWithOmdbFallback(searchTerms, typeFilter) {
        const searchPromises = searchTerms.map(term => fmdbClient.searchWithDetails(term, typeFilter, 5));
        const allResults = await Promise.all(searchPromises);
        const flatResults = allResults.flat();
        return flatResults.map(r => fmdbClient.convertToInternal(r));
    }
    normalizeSearchTerms(searchTerms, limit = RecommendationEngine.SEARCH_TERM_LIMIT) {
        return Array.from(new Set(searchTerms
            .map(term => term.trim())
            .filter(Boolean))).slice(0, limit);
    }
    async buildActorSeedCandidates(actors, description, typeFilter) {
        ...
        const seedSearches = await Promise.all(titleSeeds.map(seed => fmdbClient.searchWithDetails(seed, typeFilter, 3)));
        return seedSearches
            .flat()
            .map(item => fmdbClient.convertToInternal(item));
    }
    convertTmdbResult(item, actorHints = []) {
    ...
    async hydratePreviousRecommendations(recommendationIds) {
    ...
    async searchCandidates(searchTerms, typeFilter, discoveryMode, preferredGenres = []) {
        if (searchTerms.length === 0) {
            return { results: [], source: tmdbClient.isEnabled() ? 'tmdb' : 'omdb' };
        }
        if (tmdbClient.isEnabled()) {
            const results = await this.searchWithTmdbPrimary(searchTerms, typeFilter, discoveryMode, preferredGenres);
            if (results.length > 0) {
    ...
    buildBackfillSearchTerms(query, preferences, existingTerms) {
        ...
        return this.normalizeSearchTerms(Array.from(extras).filter(term => !existing.has(term.toLowerCase())), RecommendationEngine.BACKFILL_SEARCH_TERM_LIMIT);
    }
    matchesRequestedActors(title, requestedActors) {
    ...
    deriveReferenceGenres(referenceTitles) {
    ...
    scoreDiversityAdjustment(candidate, selected) {
        if (selected.length < 2) {
            return 0;
        }
        const candidateGenres = (candidate.genres || []).map((genre) => genre.toLowerCase());
        const selectedGenres = new Set(selected.flatMap((item) => (item.genres || []).map((genre) => genre.toLowerCase())));
        const samePrimaryGenreCount = selected.filter((item) => {
            const selectedPrimaryGenre = item.genres?.[0]?.toLowerCase();
            return selectedPrimaryGenre && selectedPrimaryGenre === candidateGenres[0];
        }).length;
        const overlapCount = selected.filter((item) => {
            const itemGenres = (item.genres || []).map((genre) => genre.toLowerCase());
            if (candidateGenres.length === 0 || itemGenres.length === 0) {
                return false;
            }
            const overlap = candidateGenres.filter((genre) => itemGenres.includes(genre)).length;
        ...
        const sameDecadeCount = candidateDecade === null
            ? 0
            : selected.filter((item) => this.getDecade(item.year) === candidateDecade).length;
        const introducesNewGenre = candidateGenres.some((genre) => !selectedGenres.has(genre));
        let adjustment = 0;
    ...
    selectFinalResults(ranked, limit) {
     ...
     */
    async getRecommendations(request, limit = 10) {
        const diagnostics = {
        ...
        if (preferences.referenceTitle && preferences.referenceTitle.length > 0) {
            ...
            const refTitlePromises = preferences.referenceTitle.map(refTitle => this.fetchReferenceTitle(refTitle));
            const refResults = await Promise.all(refTitlePromises);
            const validRefs = refResults.filter((r) => r !== null);
        ...
        const typeFilter = preferences.contentType === 'tv' ? 'series' :
            preferences.contentType === 'movie' ? 'movie' :
                undefined;
        const hasExactYearConstraint = preferences.yearRange?.min !== undefined &&
            preferences.yearRange?.max !== undefined &&
            preferences.yearRange.min === preferences.yearRange.max;
        const useBlockbusterPaging = tmdbClient.isEnabled() &&
            preferences.popularityPreference === 'mainstream' &&
            hasExactYearConstraint;
        const useCriticsYearProxy = tmdbClient.isEnabled() &&
            preferences.criticsIntent === true &&
            hasExactYearConstraint;
        let actorFilmographyCandidates = [];
        if (preferences.discoveryMode === 'talent' && preferences.detectedActors && preferences.detectedActors.length > 0 && tmdbClient.isEnabled()) {
            const includeMovies = typeFilter !== 'series';
            const includeTV = typeFilter !== 'movie';
            const actorResultSets = await Promise.all(preferences.detectedActors.map(actor => tmdbClient.searchTitlesForPerson(actor, {
                includeMovies,
                includeTV,
                excludeAdult: true
            }, 20)));
            actorFilmographyCandidates = actorResultSets.flatMap((results, index) => results.map(item => this.convertTmdbResult(item, [preferences.detectedActors[index]])));
        ...
        const shouldReusePreviousCandidates = (request.clarificationContext?.clarificationRound ?? 0) > 0 &&
        ...
        if (shouldReusePreviousCandidates) {
            ...
            console.log(`[Engine] Reuse mode: hydrated ${previousCandidates.length}/${previousRecommendationIds.length} previous recommendations`);
        }
        let candidates = [];
        if (useBlockbusterPaging) {
            ...
            const includeMovies = typeFilter !== 'series';
            const includeTV = typeFilter !== 'movie';
            const pagedMainstream = await tmdbClient.discoverPopularByYear(preferences.yearRange.min, {
                includeMovies,
                includeTV,
                excludeAdult: true
            }, page);
            ...
            diagnostics.usedTmdb = true;
            console.log(`[Engine] Blockbuster paging: fetched ${candidates.length} candidates for year ${preferences.yearRange.min} page ${page}`);
        }
        if (useCriticsYearProxy) {
            const includeMovies = typeFilter !== 'series';
            const includeTV = typeFilter !== 'movie';
            const criticsPool = await tmdbClient.discoverPopularByYear(preferences.yearRange.min, {
                includeMovies,
                includeTV,
                excludeAdult: true
            }, 1);
            ...
            diagnostics.usedTmdb = true;
            console.log(`[Engine] Critics proxy: fetched ${candidates.length} candidates for year ${preferences.yearRange.min}`);
        }
        ...
        if (!useBlockbusterPaging && !useCriticsYearProxy && shouldReusePreviousCandidates && previousCandidates.length >= limit) {
            ...
            const searched = await this.searchCandidates(searchTerms, typeFilter, preferences.discoveryMode, preferences.genres || []);
        ...
        if (!useBlockbusterPaging && !useCriticsYearProxy && candidates.length < RecommendationEngine.MIN_CANDIDATE_POOL) {
            const backfillTerms = this.buildBackfillSearchTerms(request.description, preferences, searchTerms);
            if (backfillTerms.length > 0) {
                console.log(`[Engine] Thin candidate pool (${candidates.length}); backfilling with: ${backfillTerms.join(', ')}`);
                const backfill = await this.searchCandidates(backfillTerms, typeFilter, preferences.discoveryMode, preferences.genres || []);
        ...
        if (!useBlockbusterPaging &&
            ...
            try {
                const hasYearRange = preferences.yearRange?.min !== undefined &&
                    preferences.yearRange?.max !== undefined;
                ...
                if (convertedMainstream.length > 0) {
                    ...
                    diagnostics.usedTmdb = true;
                    console.log(`[Engine] Mainstream fallback: added ${convertedMainstream.length} trending TMDB candidates`);
                }
            }
            catch (error) {
        ...
        const filteredRanked = preferences.discoveryMode === 'talent' && preferences.detectedActors && preferences.detectedActors.length > 0
            ? ranked.filter(title => this.matchesRequestedActors(title, preferences.detectedActors || []))
            : ranked;
        let effectiveRanked = filteredRanked;
        if (effectiveRanked.length === 0 &&
            ...
            const actorSeedCandidates = await this.buildActorSeedCandidates(preferences.detectedActors, request.description, typeFilter);
            if (actorSeedCandidates.length > 0) {
                ...
                const recoveredWithTalent = recoveredSafe.map(title => ({
                    ...title,
                    talentMatchScore: TalentMatcher.findTalentMatchForActors(title, preferences.detectedActors || []).combinedScore
                }));
                const recoveredRanked = this.rankTitles(recoveredWithTalent, preferences, referenceTitles);
                const recoveredFiltered = recoveredRanked.filter(title => this.matchesRequestedActors(title, preferences.detectedActors || []));
        ...
        if (llmClient.isEnabled() && final.length > 0) {
            try {
                ...
                llmExplanations = await llmClient.generateWhyThisBatch(request.description, titlesForLLM);
        ...
        return final.map(title => this.titleToRecommendation(title, preferences, request.description, llmExplanations, availabilityData, trailerData, referenceTitles));
    }
     ...
     */
    async fetchReferenceTitle(titleName) {
        // Check cache first
     ...
     */
    extractSearchTerms(query, genres, maxTerms = RecommendationEngine.SEARCH_TERM_LIMIT) {
        const terms = new Set();
        // Remove stop words
        const stopWords = [
     ...
     */
    inferCoreGenres(query, parsedGenres, referenceTitles = []) {
     ...
     */
    rankTitles(titles, preferences, referenceTitles = []) {
        // === PHASE 3.1: Hard Genre Filter ===
        // Infer core genres from query and parsed preferences
        const coreGenres = this.inferCoreGenres(preferences.description || '', preferences.genres, referenceTitles);
        ...
        const hasRewatchIntent = /(rewatch|watch again|same movie|the original|again)/i.test(preferences.description || '');
        if (!hasRewatchIntent && referenceTitles.length > 0) {
            ...
            if (filtered.length < beforeCount) {
                console.log(`[Engine.Rank] Suppressed ${beforeCount - filtered.length} explicitly-mentioned anchor title(s)`);
            }
        }
        if (coreGenres.length > 0) {
            filtered = filtered.filter(t => {
                const titleGenres = (t.genres || []).map((g) => g.toLowerCase());
                const hasRequiredGenre = coreGenres.some(cg => titleGenres.some((tg) => tg.toLowerCase() === cg.toLowerCase()));
                ...
                if (isHeistQuery && hasRequiredGenre) {
                    ...
                    if (!heistKeywords.test(titleLower) && !heistKeywords.test(plotLower)) {
                        console.log(`[Engine.Rank] Semantic filter: removed "${t.title}" - Crime/Thriller but no heist keywords`);
                        return false;
                    }
                }
            ...
            if (filtered.length < titles.length) {
                ...
                console.log(`[Engine.Rank] Hard genre filter: removed ${removed} titles without core genres or heist semantics [${coreGenres.join(', ')}]`);
            }
        }
        ...
        if (preferences.yearRange?.min !== undefined &&
            ...
            if (beforeCount !== filtered.length) {
                console.log(`[Engine.Rank] Strict year filter (${targetYear}) removed ${beforeCount - filtered.length} titles`);
            }
        }
        ...
        if (preferences.popularityPreference === 'mainstream' && filtered.length > 0) {
            ...
            if (hasExactYearConstraint) {
                ...
                if (mainstreamYearPool.length > 0) {
                    ...
                    console.log(`[Engine.Rank] Mainstream year floor kept ${filtered.length}/${beforeCount} titles (broad vote-count threshold)`);
                }
            }
            if (!hasExactYearConstraint) {
                ...
                if (blockbusterCandidates.length >= Math.min(5, beforeCount)) {
                    ...
                    console.log(`[Engine.Rank] Mainstream filter kept ${filtered.length}/${beforeCount} titles with strong popularity signals`);
                }
            }
        }
        // === PHASE 3.2: Exclusion Genre Penalty ===
        // Track exclusion penalties before scoring
        const exclusionPenalties = new Map();
        filtered.forEach(t => {
            let penalty = 1.0; // No penalty by default
            if (preferences.excludedGenres && preferences.excludedGenres.length > 0) {
                const titleGenres = (t.genres || []).map((g) => g.toLowerCase());
                const hasExcluded = titleGenres.some((tg) => preferences.excludedGenres.some(eg => eg.toLowerCase() === tg.toLowerCase()));
                if (hasExcluded) {
                    // Heavy penalty: reduce composite score to 30% of original
                    penalty = 0.3;
                    console.log(`[Engine.Rank] Exclusion penalty 0.3x: "${t.title}" has excluded genres [${(t.genres || []).join(', ')}]`);
                }
            }
        ...
        if (preferences.rankingStrategyPreference && scoringConfig) {
            if (preferences.rankingStrategyPreference === 'mood_first') {
                scoringConfig.weights = {
                    ...
                    talent: Math.max(preferences.discoveryMode === 'talent' ? 0.3 : 0.12, scoringConfig.weights.talent * 0.8),
            ...
            if (preferences.rankingStrategyPreference === 'reference_first') {
                scoringConfig.weights = {
                    ...
                    talent: Math.max(preferences.discoveryMode === 'talent' ? 0.28 : 0.2, scoringConfig.weights.talent),
        ...
        if (preferences.criticsIntent) {
            ...
            if (strictPool.length >= Math.min(5, beforeCount)) {
                filtered = strictPool;
                console.log(`[Engine.Rank] Critics proxy strict floor kept ${filtered.length}/${beforeCount} titles`);
            }
            else if (relaxedPool.length > 0) {
                filtered = relaxedPool;
                console.log(`[Engine.Rank] Critics proxy relaxed floor kept ${filtered.length}/${beforeCount} titles`);
            }
        }
        if (scoringConfig && preferences.discoveryMode) {
            console.log(`[Engine.Rank] Applying ${preferences.discoveryMode} mode weights: ` +
        ...
        const penalizedRanked = ranked.map(r => {
            ...
            return {
                ...
                scoringFactors: {
                    ...r.scoringFactors,
                    composite: Math.max(0, r.scoringFactors.composite * penalty * moodShiftMultiplier * refinementMultiplier)
                }
            };
        });
        // Re-sort after applying penalties
        const resorted = penalizedRanked.sort((a, b) => b.scoringFactors.composite - a.scoringFactors.composite);
        if (preferences.criticsIntent) {
    ...
    calculateMoodShiftMultiplier(title, preferences) {
        if (!title?.plot) {
            return 1.0;
        }
    ...
    calculateRefinementMultiplier(title, preferences) {
        let multiplier = 1.0;
     ...
     */
    titleToRecommendation(item, preferences, userDescription, llmExplanations, availabilityData, trailerData, referenceTitles) {
        ...
        const whyThis = llmWhyThis && llmWhyThis.trim() && llmWhyThis.trim().toLowerCase() !== 'null'
            ? llmWhyThis
        ...
        const scoringFactors = item.scoringFactors;
        return {
            id: item.id,
            title: item.title,
            year,
     ...
     */
    generateWhyThis(item, preferences) {
        ...
        if (preferences.genres.length > 0) {
            const matchedGenres = (item.genres || []).filter((g) => preferences.genres.some(pg => pg.toLowerCase() === g.toLowerCase()));
        ...
        if (preferences.excludedGenres && preferences.excludedGenres.length > 0) {
            const hasExcluded = (item.genres || []).some((g) => preferences.excludedGenres.some(eg => eg.toLowerCase() === g.toLowerCase()));
            if (!hasExcluded) {
...
RecommendationEngine.SEARCH_TERM_LIMIT = 6;
RecommendationEngine.BACKFILL_SEARCH_TERM_LIMIT = 3;
RecommendationEngine.MIN_CANDIDATE_POOL = 12;
RecommendationEngine.DIVERSITY_WINDOW_MULTIPLIER = 3;
